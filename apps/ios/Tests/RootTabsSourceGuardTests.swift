...
struct RootTabsSourceGuardTests {
    ...
    @Test func `workboard create action surfaces unavailable reasons`() throws {
        ...
        #expect(source.contains("state.activeRefreshBoardID = boardScope.boardID.map { .init(value: $0.value) }"))
        ...
        #expect(source.contains("state.isCreatingCard = .init(value: true)"))
        #expect(source.contains("state.isCreatingCard = .init(value: false)"))
        #expect(source.contains(".disabled(self.store.isCreatingCard.value)"))
        #expect(source.contains("self.store.isCreatingCard.value ? \"Creating...\" : \"Create\""))
        ...
        #expect(source.contains("self.isRefreshing.value || self.isDispatching.value"))
        #expect(source.contains("state.isRefreshing = .init(value: true)"))
        #expect(source.contains("state.isRefreshing = .init(value: false)"))
        ...
        #expect(source.contains("state.isDispatching = .init(value: true)"))
        #expect(source.contains("state.isDispatching = .init(value: false)"))
    ...
    @Test func `skill workshop uses kanban lanes on wide I pad`() throws {
        ...
        #expect(source.contains("guard !state.isLoading.value else { return .none }"))
        ...
        #expect(source.contains("struct ProposalSheetOpening: Equatable, Sendable"))
        #expect(source.contains("var readAccess: GatewayReadAccess"))
    ...
    @Test func `native chat uses gateway transport`() throws {
        ...
        #expect(channelsSource.contains("actionIcon: self.store.isLoading.value ? \"hourglass\" : \"arrow.clockwise\""))
        #expect(channelsSource.contains("isActionDisabled: self.store.isLoading.value"))
        #expect(channelsSource.contains("if self.store.isLoading.value, self.store.entries.isEmpty"))
    ...
    @Test func `settings location apply request is reducer effect owned`() throws {
        ...
        #expect(locationSource.contains("struct LocationModeChangeInFlight: Equatable, Sendable { var value: Bool }"))
    ...
    @Test func `settings agent selection persistence is reducer effect owned`() throws {
        ...
        #expect(!settingsSource.contains("selectedAgentPickerId: String"))
