...
struct RootTabsSourceGuardTests {
    ...
    @Test func `workboard create action surfaces unavailable reasons`() throws {
        ...
        #expect(source
            .contains("private var compactWriteUnavailablePresentation: IPadWorkboardCompactWriteUnavailablePresentation"))
        #expect(source.contains("self.store.state.compactEmptyStatePresentation(canRead: self.canRead)"))
        #expect(source.contains("self.store.state.compactWriteUnavailablePresentation(canRead: self.canRead)"))
