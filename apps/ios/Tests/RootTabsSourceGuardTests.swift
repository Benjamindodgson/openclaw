...
struct RootTabsSourceGuardTests {
    ...
    @Test func `workboard create action surfaces unavailable reasons`() throws {
        ...
        #expect(source.contains("state.activeRefreshBoardID = boardScope.boardID.map { .init(value: $0) }"))
        #expect(source.contains("let activeRefreshBoardID = response.boardScope.boardID"))
        #expect(source.contains(".map { IPadWorkboardActiveRefreshBoardID(value: $0) }"))
        ...
        #expect(source.contains("state.draftNotes = change.notes.value"))
        #expect(source.contains("state.draftTitle = change.title.value"))
