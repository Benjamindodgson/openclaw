...
struct RootTabsSourceGuardTests {
    ...
    @Test func `workboard create action surfaces unavailable reasons`() throws {
        ...
        #expect(source.contains("options: Self.statusFilterOptions(for: self.statusValues)"))
        #expect(source.contains("compactOptions: Self.statusFilterOptions(for: self.compactStatuses)"))
