...
struct SettingsPresentationFeature {
    ...
    struct State: Equatable, Sendable {
        enum Destination: Equatable, Sendable {
            ...
            case notificationRelayDisclosure
            case qrScanner
            case scannerError(SettingsPresentationScannerErrorMessage)
            case resetOnboardingAlert
        ...
        var showNotificationRelayDisclosure: Bool {
            self.destination == .notificationRelayDisclosure
        }
        ...
        var showResetOnboardingAlert: Bool {
            self.destination == .resetOnboardingAlert
        }
    ...
    var body: some ReducerOf<Self> {
        Reduce { state, action in
            ...
            case .notificationRelayDisclosureRequested:
                state.destination = .notificationRelayDisclosure
                return .none

            case .notificationRelayDisclosureDismissed:
                if state.destination == .notificationRelayDisclosure {
                    state.destination = nil
                }
                return .none
            ...
            case .resetOnboardingButtonTapped:
                state.destination = .resetOnboardingAlert
                return .none

            case .resetOnboardingAlertDismissed:
                if state.destination == .resetOnboardingAlert {
                    state.destination = nil
                }
                return .none
