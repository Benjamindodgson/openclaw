...
extension SettingsProTab {
    ...
    func runDiagnostics() async {
        guard self.gatewayActivityStore.refreshPhase != .inFlight else { return }
        let isAppleReviewDemoModeEnabled = self.appModel.isAppleReviewDemoModeEnabled
        await self.gatewayActivityStore
    ...
    func syncSettingsState() {
        ...
        self.appearanceStore.send(.appearancePreferenceSynced(.init(
            rawValue: .init(value: self.storedAppearancePreferenceRaw))))
    ...
    func syncNotificationRelayState() {
        ...
        self.notificationStore.send(.relayConfigSynced(.init(
            usesOpenClawHostedRelay: .init(value: config.usesOpenClawHostedRelay),
            hostedRelayHost: .init(value: host))))
    }
    ...
    func connect(_ gateway: GatewayDiscoveryModel.DiscoveredGateway) async {
        self.gatewayConnectionStore.send(.connectionStarted(.init(gatewayID: .init(value: gateway.id))))
    ...
    func connectManual() async {
        ...
        case let .request(request):
            self.gatewayConnectionStore.send(.connectionStarted(.init(gatewayID: .init(value: "manual"))))
    ...
    func handleLocationModeApplyResult(_ result: SettingsLocationFeature.LocationModeApplyResult?) {
        ...
        if case let .denied(denied) = result {
            self.storedLocationModeRaw = denied.previousValue.rawValue
        }
    }
    ...
    func requestNotificationAuthorizationFromSettings() {
        guard self.notificationStore.authorizationRequestPhase != .inFlight else { return }
    ...
    func updateVoiceWakeEnabled(_ enabled: Bool) {
        self.voiceControlStore.send(.voiceWakeEnabledChanged(.init(
            enabled: .init(isEnabled: enabled))))
